                        ...
                        @foreach ($categories as $category)
                            <tr>
                                <td>
                                    ...
                                    style="width: 75px; height: 85px; border-radius: 50%;">
                                </td>
                                <td>{!! wrapText($category->name, 30) !!}</td>
                                <td>{!! wrapText($category->description, 30) !!}</td>
                                <td>
                                    <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                        data-bs-target="#restoreModal{{ $category->id }}">
                                        <i class="fas fa-undo"></i>
                                    </button>
                                    <!-- Restore Confirmation Modal -->
                                    <div class="modal fade" id="restoreModal{{ $category->id }}" tabindex="-1"
                                        aria-labelledby="restoreModalLabel{{ $category->id }}" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="restoreModalLabel{{ $category->id }}">
                                                        Restore Category</h5>
                                                ...
                                                <div class="modal-footer">
                                                    <form action="{{ route('categories.restore', $category->id) }}"
                                                        method="POST" id="restore-form-{{ $category->id }}">
                                                        @csrf
                                                        <button type="button" class="btn btn-secondary"
                                                            data-bs-dismiss="modal">Cancel</button>
                                                        <button type="button" class="btn btn-success"
                                                            onclick="event.preventDefault(); document.getElementById('restore-form-{{ $category->id }}').submit();">Restore</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="modal fade" id="deleteModal{{ $category->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <p>Are you sure you want to delete this category? This action cannot be
                                                        undone.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <form action="{{ route('categories.forceDelete', $category->id) }}"
                                                        ...
                                                        <button type="button" class="btn btn-danger"
                                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $category->id }}').submit();">Delete</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    @else
                        <tr>
                            <td colspan="4" class="text-center">
                                No trashed categories found. (back to <a
